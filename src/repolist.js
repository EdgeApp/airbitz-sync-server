...
async function main() {
  while (1) {
    ...
    const stdoutString = childProcess.execFileSync(
      'find',
      [reposDir, '-maxdepth', '2', '-mindepth', '2'],
      {
        // stdio: std,
        cwd: config.userDir,
        killSignal: 'SIGKILL'
    ...
    fs.createReadStream(config.userDir + 'repolist.tmp').pipe(
      fs.createWriteStream(config.repoListPath + 'repolist.txt')
    )

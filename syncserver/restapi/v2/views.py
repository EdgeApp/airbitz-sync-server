...
def git_update(path, storeId, changes, start_hash=None):
    ...
    check_output(["git", "--git-dir={0}".format(path), '--work-tree={0}'.format(working_tree), "reset"], cwd=working_tree)
    for k,v in changes.iteritems():
        ...
        f.close()
        check_output(["git", "--git-dir={0}".format(path), '--work-tree={0}'.format(working_tree), "add", "--", filename], cwd=working_tree)
    if git_dirty(path, working_tree):
        check_output(["git", "--git-dir={0}".format(path), '--work-tree={0}'.format(working_tree), "commit", "-m", "web commit"], cwd=working_tree)
    else:
